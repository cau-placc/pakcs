...
shellCmdWithCurryPath(Cmd) :-
	localCurryPath(CPS),
        getSysLibPath(SysLib), path2String(SysLib,SysLibS),
        ((CPS="" ; SysLibS="") -> append(CPS,SysLibS,FullPathS)
                                ; append(CPS,[58|SysLibS],FullPathS)),
	(FullPathS="" -> Export=[]
          ; concat(["CURRYPATH='",FullPathS,"' && export CURRYPATH && "],
                   Export)),
	atom_codes(Cmd,CmdS),
