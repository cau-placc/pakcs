...
If the initial expression is of functional type or an I/O action,
...
Instead of the show mode, one can also write the initial expression
\begin{curry}
print $exp$
\end{curry}
if it is ensured that $exp$ evaluates deterministically
to a data term without free variables.
...
Time mode. If the time mode is on, the cpu time and the elapsed time
