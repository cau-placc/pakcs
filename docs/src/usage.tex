...
\item[\fbox{:main}]\pindex{:main}
Execute the function \code{main} of the current module.
It is assumed (but not checked) that this function exists and
is of type \code{IO ()}.
This command is useful to execute the main function of a module
where the source file does not exist, i.e., which is loaded from
a FlatCurry file.
