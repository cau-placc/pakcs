...
If the option \code{--code} is set, the full path containing
all source directories is stored as property
\code{curry.languageServer.importPaths}
in the file \ccode{.vscode/settings.json}
(which is updated if it already exists)
so that all imported modules of the current package can be found
by the Curry Language Server%
